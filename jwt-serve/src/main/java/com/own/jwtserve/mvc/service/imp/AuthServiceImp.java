package com.own.jwtserve.mvc.service.imp;
...
public class AuthServiceImp implements AuthService{
    ...
    public String authentication(String username, String password) {
        ...
        try{
            //1.调用微服务查询用户信息
            ResponseEntity<SysUser> res = this.userClient.queryUser(username,password);
            //2.查询结果为空，则直接返回null
            if (!res.getStatusCode().equals(HttpStatus.OK)){
                return null;
            }
            SysUser user = res.getBody();
            String token = null;
            //3.查询结果不为空，则生成token
            /*String token = JwtUtils.generateToken(new UserInfo(user.getId(), user.getName()),
    ...
    private AuthToken applyToken(String username, String password, String clientId, String clientSecret){
        ...
        String authUrl = "http://JWT-SERVE"+ "/auth/oauth/token";
        //定义header
